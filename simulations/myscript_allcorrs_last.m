...
function[] = myscript_allcorrs_last(seed,scaling_factor)
...
Num_samples = 4000;
matrix_size = Ne/2 * Ne/2;
permut_corr = randperm(matrix_size,Num_samples);

All_corrs_laser = zeros(NBlockTrials,Num_samples);
All_corrs_laser_early = zeros(Ne/2,Ne/2);
All_corrs_laser_late = zeros(Ne/2,Ne/2);
...
parfor BlockTrial=1:NBlockTrials
    for window = 1:Number_of_points_corr
        ...
        if(ComputeSpikeCountCorrs)
            ...
            toc
            if( window == 12 )
                temp = R(1:Ne/2,1:Ne/2);
                All_corrs_laser(BlockTrial,:) = temp(permut_corr);
            end
            
            if( window == 12 && BlockTrial < 6 )
                All_corrs_laser_early = All_corrs_laser_early + R(1:Ne/2,1:Ne/2);
            end

            if( window == 12 && BlockTrial > 35 )
                All_corrs_laser_late = All_corrs_laser_late + R(1:Ne/2,1:Ne/2);
            end
...
All_corrs_laser_early = All_corrs_laser_early/5;
All_corrs_laser_late = All_corrs_laser_late/5;
...
All_corrs_control = zeros(NBlockTrials,Num_samples);
All_corrs_control_early = zeros(Ne/2,Ne/2);
All_corrs_control_late = zeros(Ne/2,Ne/2);
...
%            Variance_stderr_nonlaserPairs_Control(BlockTrial,window)=std(Variance( find(StimulatedNeurons==0)))/sqrt((Ne-sum(StimulatedNeurons)));
		...
		if( window == 12 )
                    temp = R(1:Ne/2,1:Ne/2);
                    All_corrs_control(BlockTrial,:) = temp(permut_corr);
                end
            
                if( window == 12 && BlockTrial < 6 )
                    All_corrs_control_early = All_corrs_control_early + R(1:Ne/2,1:Ne/2);
                end

                if( window == 12 && BlockTrial > 35 )
                    All_corrs_control_late = All_corrs_control_late + R(1:Ne/2,1:Ne/2);
            	end
...
All_corrs_control_early = All_corrs_control_early/5;
All_corrs_control_late = All_corrs_control_late/5;
...
save( ['/scratch/AlanAkil/ArianaVariables_ee_ei_ie_ii_seed_' num2str(seed) '_scale_' num2str(scaling_factor) '_freq_' num2str(laser_freq) '_eeSTDP_allcorrs.mat'], 'seed',...
    ...
    'PSTH_laserPairs_Laser','PSTH_laserPairs_Control',...
    'Number_of_points_PSTH',...
    'IeCurrent','IiCurrent','IxCurrent',...
    'All_corrs_laser','All_corrs_laser_early','All_corrs_laser_late',...
    'All_corrs_control','All_corrs_control_early','All_corrs_control_late',...
    'permut_corr','Num_samples')
